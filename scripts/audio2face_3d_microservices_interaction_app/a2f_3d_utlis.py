...
def compute_output_directory_name(save_name: str=None) -> str:
    if save_name:
        return f"{OUTPUT_BASE_FOLDER}_{save_name}"
    index = 1
...
async def read_from_stream(stream, should_print_to_files, print_fps, file_name):
    ...
    while True:
        ...
        if message == grpc.aio.EOF:
            ...
            if should_print_to_files:
                dir_name = compute_output_directory_name("bs")
                ...
                # save_audio_data_to_file(dir_name, audio_header, audio_buffer)  # 屏蔽

                # Normalize the dictionary data to output in JSON.
                # df_animation = pd.json_normalize(animation_key_frames)
                # df_smoothed_output = pd.json_normalize(emotion_key_frames["a2f_smoothed_output"])  # 屏蔽
                # df_input = pd.json_normalize(emotion_key_frames["input"])  # 屏蔽
                save_bs = f"{dir_name}/{file_name}.json"
                with open(save_bs, "w") as bs_file:
                    bs_file.write(json.dumps(animation_key_frames, indent=4))
                # Save data to csv.
                # print(f"Saving data into {dir_name} folder...")
                print(f"Saving data to {save_bs} ...")
                # df_animation.to_csv(f"{dir_name}/animation_frames.csv")
                # df_smoothed_output.to_csv(f"{dir_name}/a2f_3d_smoothed_emotion_output.csv")  # 屏蔽
                # df_input.to_csv(f"{dir_name}/a2f_3d_input_emotions.csv")  # 屏蔽
            return
        ...
        if message.HasField("animation_data_stream_header"):
            ...
            # parse_emotion_data(animation_data, emotion_key_frames)  # 屏蔽
